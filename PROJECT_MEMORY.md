...
- `web/src/components/editor/ResumePreview.tsx`: right-side live preview plus application-level large preview dialog, fit scaling, max-two-page behavior, paper accent color from `draft.theme.accentColor`, and readable rendering of common Markdown in long text fields. It consumes `createResumePaperLayoutPlan()` and renders each `PaperBlock` page directly; page-count decisions must remain planner-driven rather than DOM measurement-driven.

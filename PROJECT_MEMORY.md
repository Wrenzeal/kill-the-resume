...
- Backend CORS defaults now include localhost dev origins plus `https://*.vercel.app`, with Gin wildcard support enabled. Browser extension origins are allowed by route-aware CORS only for `POST /api/v1/job-radar/import` preflight (`chrome-extension://` / `moz-extension://`), while token-management/account routes remain blocked for extension origins; production was deployed and verified on 2026-06-24. Production `/var/www/kill-the-resume/shared/backend.env` currently includes `https://killer.wrenzeal.top`, `https://*.vercel.app`, and `https://api.killer.wrenzeal.top`; after binding any additional custom frontend domain, add that exact origin to `CORS_ORIGINS`. Vercel rewrites are preferred because they keep browser API calls same-origin.
...
- Resume output now follows a “Developer Dossier / 技术证据型” direction: `draft.identity.highlights` stores one-per-line role-match signals rendered near the top as dedicated evidence chips in both live preview and vector PDF; old drafts missing the field normalize to an empty string. Default project copy uses Problem/Decision/Impact prompts, and default skills are framed as Production / Frontend Systems / Backend/Data / Engineering Signals rather than a generic skill dump.
...
- Custom module preview/PDF export share `projectCustomModuleSection` from `web/src/lib/resume-projection.ts`; newly added custom modules render their section title even before field values are filled, and visible custom text/textarea/date fields are projected consistently for live preview and vector PDF export.
- Preview auto-fits content and supports up to two A4 pages. If content exceeds the intended density, it shows compression/risk signals rather than silently overflowing. Module titles, item titles, field captions, skill category subtitles, and identity evidence highlights in preview/PDF are intentionally larger/bolder than body text to preserve clear resume hierarchy.
- Right-side live preview is now a continuous long paper surface, not paginated A4; it renders `layoutPlan.blocks` once and inserts dashed PDF page-guide markers based on `layoutPlan.pages`. Large preview and vector PDF export remain the paginated A4/PDF-style views. Preview density is applied through inherited `--resume-density-scale` font/spacing styles; do not reintroduce whole-page `scaleY()` compression because the planner block heights are already scaled and double-scaling causes squeezed page ends/false whitespace. Do not reintroduce DOM `scrollHeight`/module-count pagination or standalone PDF compression heuristics; update the shared planner when changing resume output density.
...
- The page keeps the Tactical Terminal visual language and uses a three-column workbench layout: target profile, opportunity priority queue, and decision panel. The queue supports status filters/actions for logged-in users (`new`, `saved`, `applying`, `applied`, `archived`, `rejected`); anonymous users can still browse the public feed but cannot persist workflow state. The decision panel shows score breakdown, recommendation tags, and risk/gap tags. The home page has a `启动机会雷达` / `Launch Job Radar` entry.
...
- `web/src/lib/job-radar.ts` owns pure frontend types/functions for default criteria, criteria normalization, percentage matching helpers, score breakdowns, match tags, warning tags, source URL preservation, freshness status, and expired-job filtering. It no longer contains product mock postings. The production feed is fetched through `web/src/lib/api.ts` using `buildJobRadarJobsPath` / `apiClient.listJobRadarJobs`; authenticated import uses `apiClient.importJobRadarPosting`; authenticated workflow updates use `apiClient.updateJobRadarJobState`; the frontend supports `refresh=1` via the `刷新岗位源` / `Refresh Job Source` button for the current search scope, sends refresh requests with browser `no-store`, and displays the online fetch/link counts returned by the backend.
...
- Backend opportunity-radar data lives in `backend/internal/jobradar`: Remotive public API client, repository/upsert, per-search on-demand sync, expiration cleanup, user preference persistence, per-user job workflow state, and Go-side matching. `backend/internal/models.JobPosting` is the canonical source-job cache and stores `sourceName`, `sourceJobId`, `sourceUrl`, `postedAt`, `firstSeenAt`, `lastSeenAt`, `expiresAt`, and `freshnessStatus`; `JobSearchCache` stores normalized search criteria plus `lastSyncedAt`; `JobSearchResult` links a search fingerprint to cached jobs so changing role keywords (for example frontend -> backend) creates/uses an independent result set instead of the previous global cache; `JobRadarPreference` stores one normalized latest search criteria record per user; `JobRadarPluginToken` stores per-user browser-extension token metadata/hash for scoped import authentication; `JobRadarJobState` stores logged-in users' per-job workflow status (`new`, `saved`, `applying`, `applied`, `archived`, `rejected`), note, priority, and next-action timestamp keyed by user + job. Default sync interval is 360 minutes per search fingerprint; `excludeKeywords` and `minScore` only affect local filtering/scoring and do not create a new source-fetch scope. `refresh=1` / `forceRefresh=true` is strict online refresh: Remotive requests use no-cache headers plus a cache-buster, only current-scope Remotive online-source links are replaced by this fetch, Boss/plugin/manual import links in the same search fingerprint are preserved, and source failure returns an error instead of falling back to old cached rows. Chinese role/location/company tokens such as `后端`, `前端`, `远程`, `深圳`, `外企` are expanded to English source/matching terms for the Remotive backend. Keep Remotive source attribution and source URL links visible because their public API terms require source mention and link-back, and avoid high-frequency polling.
...
- `web/src/components/editor/ResumePreview.tsx`: right-side continuous long preview plus application-level large paginated A4 preview dialog, density-aware preview typography/spacing, paper accent color from `draft.theme.accentColor`, and readable rendering of common Markdown in long text fields. It consumes `createResumePaperLayoutPlan()`; side preview uses `layoutPlan.blocks` with PDF page-guide dividers, while large preview uses `layoutPlan.pages`. Page-count decisions must remain planner-driven rather than DOM measurement-driven, and preview paper content must not use `scaleY()` for density.
...
- `job-radar-extension/`: standalone Chrome/Edge Manifest V3 extension project for Opportunity Radar. It uses `activeTab`, `scripting`, and `storage` only after the user clicks the extension, reads the current job-detail page URL/title/canonical/page text/selected text, auto-fills posting fields with a Boss-first adapter plus Liepin/generic fallbacks, filters obvious salary mojibake before falling back to body salary extraction, infers Job Keywords / Radar Locations / Skills from Boss/Liepin titles, addresses, tags, and body text, and posts to `/api/v1/job-radar/import` with a `ktrp_` plugin token generated from the `/job-radar` page. Backend CORS intentionally allows `chrome-extension://` / `moz-extension://` only for `POST /api/v1/job-radar/import` preflight, not for token-management/account routes. Manual fields remain as correction fallback, not the primary flow. It includes a Chinese README and `popup.test.mjs` regression check for local installation, permissions, token setup, salary parsing, and development checks.

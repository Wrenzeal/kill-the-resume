...
- Right-side A4 preview, large preview dialog, and vector PDF export now share `web/src/lib/resume-paper-layout.ts` for paper metrics, density scale, pagination, and block ownership. The preview renders `PaperBlock[]` pages directly, so a long single module can continue onto page 2 instead of being clipped as an indivisible module. Preview density is applied through inherited `--resume-density-scale` font/spacing styles; do not reintroduce whole-page `scaleY()` compression because the planner block heights are already scaled and double-scaling causes squeezed page ends/false large-preview whitespace. Do not reintroduce DOM `scrollHeight`/module-count preview pagination or standalone PDF compression heuristics; update the shared planner when changing resume output density.
...
- `web/src/components/editor/ResumePreview.tsx`: right-side live preview plus application-level large preview dialog, density-aware preview typography/spacing, max-two-page behavior, paper accent color from `draft.theme.accentColor`, and readable rendering of common Markdown in long text fields. It consumes `createResumePaperLayoutPlan()` and renders each `PaperBlock` page directly; page-count decisions must remain planner-driven rather than DOM measurement-driven, and preview paper content must not use `scaleY()` for density.

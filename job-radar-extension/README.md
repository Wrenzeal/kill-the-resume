...
- 使用主站生成的 `ktrp_` 插件专用 Token 调用后端导入接口，不需要复制登录态 Token。
...
- `Plugin Token`
  - 登录 Kill The Resume 主站，进入编辑器的 **账户与云端简历 / 机会雷达插件 Token** 面板。
...
1. 登录 Kill The Resume，在编辑器账号面板生成并复制 `ktrp_` 插件 Token；确认机会雷达当前搜索条件是你想要的方向。
...
- 插件 Token 可以在主站账号面板撤销；如果怀疑泄露，立即撤销并重新生成。
